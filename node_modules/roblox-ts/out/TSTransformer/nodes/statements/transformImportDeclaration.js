        ...
        if (importClause.name) {
            ...
            if (state.resolver.isReferencedAliasDeclaration(importClause) && (!symbol || (0, isSymbolOfValue_1.isSymbolOfValue)(symbol))) {
                ...
                if (moduleSymbol && state.getModuleExports(moduleSymbol).some(v => v.name === "default")) {
                    luau_ast_1.default.list.pushList(statements, (0, transformVariableStatement_1.transformVariable)(state, importClause.name, luau_ast_1.default.property(importExp.get(), "default"))[1]);
                }
                else {
                    luau_ast_1.default.list.pushList(statements, (0, transformVariableStatement_1.transformVariable)(state, importClause.name, importExp.get())[1]);
                }
            }
        }
        if (importClause.namedBindings) {
            if (typescript_1.default.isNamespaceImport(importClause.namedBindings)) {
                luau_ast_1.default.list.pushList(statements, (0, transformVariableStatement_1.transformVariable)(state, importClause.namedBindings.name, importExp.get())[1]);
            }
            else {
                for (const element of importClause.namedBindings.elements) {
                    const symbol = state.getOriginalSymbol(element.name);
                    if (state.resolver.isReferencedAliasDeclaration(element) && (!symbol || (0, isSymbolOfValue_1.isSymbolOfValue)(symbol))) {
                        luau_ast_1.default.list.pushList(statements, (0, transformVariableStatement_1.transformVariable)(state, element.name, luau_ast_1.default.property(importExp.get(), ((_a = element.propertyName) !== null && _a !== void 0 ? _a : element.name).text))[1]);
                    }
                }
            }
        }
