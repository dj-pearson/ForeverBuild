...
function transformLuaTupleDestructure(state, bindingPattern, value) {
    return state.capturePrereqs(() => {
        ...
        if (luau_ast_1.default.list.isEmpty(ids)) {
            state.prereqList((0, wrapExpressionStatement_1.wrapExpressionStatement)(value));
        }
        else {
            state.prereq(luau_ast_1.default.create(luau_ast_1.default.SyntaxKind.VariableDeclaration, { left: ids, right: value }));
        }
...
function transformVariableDeclaration(state, node) {
    ...
    if (typescript_1.default.isIdentifier(node.name)) {
        luau_ast_1.default.list.pushList(statements, transformVariable(state, node.name, value)[1]);
    }
    else {
        (0, assert_1.assert)(node.initializer && value);
        const name = node.name;
        if (typescript_1.default.isArrayBindingPattern(name)) {
            if (luau_ast_1.default.isCall(value) && (0, types_1.isLuaTupleType)(state)(state.getType(node.initializer))) {
                luau_ast_1.default.list.pushList(statements, transformLuaTupleDestructure(state, name, value));
            }
            else {
